...
class DocenteService extends MainService{
    ...
    function insertTarea($codAsignatura, $codNivel, $codPeriodo, $codDocente, $codParalelo, $tema, $fecha, $detalle, $hora){
        $stmt = $this->conex->prepare("INSERT INTO TAREA_ASIGNATURA (COD_ASIGNATURA,  COD_NIVEL_EDUCATIVO, COD_PERIODO_LECTIVO, COD_DOCENTE, COD_PARALELO, TEMA_TAREA, FECHA_ENTREGA, DETALLE_TAREA, HORA_ENTREGA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiiiissss", $codAsignatura, $codNivel, $codPeriodo, $codDocente, $codParalelo, $tema, $fecha, $detalle, $hora);
        $stmt->execute();
        $stmt->close();
    }
    //INSERTAR COMUNICADO ASIGNATURA
    function insertComunicado($codAsignatura, $codNivel, $codPeriodo, $codDocente, $codParalelo, $asunto, $detalle, $fecha){
        $stmt = $this->conex->prepare("INSERT INTO COMUNICADO_ASIGNATURA (COD_ASIGNATURA,  COD_NIVEL_EDUCATIVO, COD_PERIODO_LECTIVO, COD_DOCENTE, COD_PARALELO, ASUNTO_COMUNICADO, DETALLE_COMUNICADO, FECHA) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiiiisss", $codAsignatura, $codNivel, $codPeriodo, $codDocente, $codParalelo, $asunto, $detalle, $fecha);
        $stmt->execute();
        $stmt->close();
    }
